...
export const PersonInfo: FC<IFCPropsWithReadonly> = ({ readonly }) => {
  ...
  const handleInputChange = useCallback(
    (field: FieldPath<IPerson>) => (event: ChangeEvent<HTMLInputElement>) => {
      if (!readonly) {
  ...
  const handleDateCalendarChange = useCallback(
    (date?: Date) => {
      if (readonly || !date) {
        return;
      }
  ...
  return (
    <Box>
      ...
      <Box sx={birthDateWrapperStyles}>
        <Typography>2. Дата народження</Typography>
        <Box>
          <CustomDatePicker onChange={handleDateCalendarChange}>
            <Input
              sx={birthDateInputStyles}
              inputProps={{ sx: { textAlign: "center", p: 0 } }}
              value={formatDateWithoutDots(convertNullOrNumberToDate(person.birthDate)) ?? ""}
            />
          </CustomDatePicker>
          {birthDateError !== undefined && (
            <Typography color="error">{birthDateError}</Typography>
          )}
        </Box>
      </Box>
      <Box sx={addressWrapperStyles}>
        <Typography>3. Місце проживання хворого: область</Typography>
        <Box>
          <Input
      ...
      <Box sx={addressWrapperStyles}>
        <Typography>місто(село)</Typography>
        <Box>
          <Input
