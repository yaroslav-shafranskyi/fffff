...
import { DateRange, SortOrder } from '../../interfaces';
...
export const QueryInfo = <T extends object>(props: IQueryInfoProps<T>) => {
    ...
    const removeFilterBy = useCallback((key: string) => () => {
        onChange('filterBy')({...filterBy, [key]: undefined });
    }, [filterBy, onChange]);

    const { optionTitle: sortOptionTitle, infoTitle: sortInfoTitle } = useMemo(() => {
        if (!sortBy || !sortsData) {
    ...
    const filtersInfo = useMemo(() => filtersData?.reduce((acc: [string, string, string][], cur) => {
        cur.fields.forEach(field => {
            ...
            const formattedTitle = key === 'Any' ? 'Загальний пошук' : title ?? key;
            if (type === TableFilterType.DATE) {
                const convertedValue = formatDate(value as Date);
                acc.push([key, formattedTitle, convertedValue]);
            }
            if (type === TableFilterType.DATE_RANGE) {
                const formattedValues: string[] = [];
                (value as DateRange).forEach(field => {
                    if (field) {
                        formattedValues.push(formatDate(field))
                    }});
                const convertedValue = formattedValues.length === 1 ? formattedValues[0] : formattedValues.join(' - '); 
                acc.push([key, formattedTitle, convertedValue]);
            }
    ...
    return (
        <Stack direction='row' spacing={1}>
            {sortOptionTitle !== undefined && <Chip
